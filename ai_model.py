...
class AI_MODEL:
    ...
    def load_model(self):
        try:
            instructions: str
            with open(
                f"prompts/{self.indications}.txt", mode="r", encoding="utf-8"
            ) as file:
                instructions = file.read()
            return GenerativeModel(
                model_name="gemini-2.5-flash",
                generation_config=GenerationConfig(
                    temperature=0.8, response_mime_type=self.results_type.value
                ),
    ...
    def generate_with_docs(self, prompt: str, context: ContextFromModel) -> str:
        parts_list = [
            Part.from_uri(
                f"gs://iq-strategy.appspot.com/{file}", mime_type="application/pdf"
            )
